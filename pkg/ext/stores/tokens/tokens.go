...
func (t *TokenStore) checkForManageToken(verb string, token *ext.Token, ctx extcore.Context) (bool, error) {
	if token.Spec.UserID == ctx.User.GetName() {
		return false, nil
	}
	isadmin, err := t.userHasManageTokenPermissions(ctx)
	if err != nil {
	...
	if !isadmin {
		return false, fmt.Errorf("cannot %s token for other user %s since user %s does not have full permissons on tokens", verb, ctx.User.GetName(), token.Spec.UserID)
	}
	return true, nil
}
